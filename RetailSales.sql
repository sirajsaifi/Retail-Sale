...
select * from retail_sales where quantity is null and cogs is null;
...
select category, gender, count(transactions_id) from retail_sales group by gender, category order by category, gender;

-- Q.7 Write a SQL query to calculate the average sale for each month. Find out best selling month in each year
select extract(YEAR from sale_date) as Year, extract(MONTH from sale_date) as Month, avg(total_sale) from retail_sales group by 1, 2 order by 1, 2;
